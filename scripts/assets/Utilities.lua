...
local u2 = {}
function Utilities.class(p1, p2)
	p1 = p1 or {};
	p1.__index = p1;
	local v7 = nil;
	if p1.className then
		v7 = {
			name = p1.className, 
			class = p1, 
	...
	function p1.new(p3, p4, ...)
		p4 = p4 or {};
		if type(p4) == "table" and not getmetatable(p4) then
			setmetatable(p4, p1);
		end;
		if p2 then
			p4 = p2(p4, ...) and p4;
...
function Utilities.subclass(p5, p6, p7)
	p6 = Utilities.class(p6, p7);
	p6.super = p5;
	function p6.__index(p8, p9)
		return rawget(p6, p9) or p5[p9];
	end;
	return p6;
end;
