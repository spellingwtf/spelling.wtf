...
local function u1(p1, p2)
	return (p1["Right Arm"].Position - p2).Magnitude;
end;
return function(p3, p4, p5)
	...
	local v2 = {};
	table.insert(v2, { workspace:Raycast(p3["Right Arm"].Position, p3.PrimaryPart.CFrame.LookVector * 10, v1), 0 });
	table.insert(v2, { workspace:Raycast(p3["Right Arm"].Position, (p3.PrimaryPart.CFrame * CFrame.Angles(0, -math.rad(1), 0)).LookVector.Unit * 10, v1), -1 });
	table.insert(v2, { workspace:Raycast(p3["Right Arm"].Position, (p3.PrimaryPart.CFrame * CFrame.Angles(0, math.rad(1), 0)).LookVector.Unit * 10, v1), 1 });
	table.insert(v2, { workspace:Raycast(p3["Right Arm"].Position, (p3.PrimaryPart.CFrame * CFrame.Angles(0, math.rad(2), 0)).LookVector.Unit * 10, v1), 2 });
	table.insert(v2, { workspace:Raycast(p3["Right Arm"].Position, (p3.PrimaryPart.CFrame * CFrame.Angles(0, -math.rad(2), 0)).LookVector.Unit * 10, v1), -2 });
